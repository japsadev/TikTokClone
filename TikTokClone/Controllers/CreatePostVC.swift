...
class CreatePostVC: UIViewController {
	...
	@IBAction func saveButtonDidTapped(_ sender: Any) {
		...
		let previewVC = UIStoryboard(name: "Main", bundle: .main).instantiateViewController(identifier: "PreviewCapturedVC", creator: {
			coder -> PreviewCapturedVC? in
			PreviewCapturedVC(coder: coder, recordedClips: self.recordedClips)
		})
	...
	@IBAction func discardButtonDidTapped(_ sender: Any) {
		...
		let discardAction = UIAlertAction(title: "Discard", style: .default) { [weak self] (_) in
			self?.handleDiscardLastRecordClip()
		}
	...
	func handleDiscardLastRecordClip() {
		print("discard")
		outPutURL = nil
		thumbnailImage = nil
	...
	func handleResetAllVisibilityToIdendity() {
		...
		if recordedClips.isEmpty == true {
			...
			print("recorded clips:", "is not Empty")
		}
...
extension CreatePostVC: AVCaptureFileOutputRecordingDelegate {
	...
	func fileOutput(_ output: AVCaptureFileOutput, didStartRecordingTo fileURL: URL, from connections: [AVCaptureConnection]) {
		let newRecordClip = VideoClips(videoUrl: fileURL, cameraPosition: currentCameraDevice?.position)
		recordedClips.append(newRecordClip)
...
extension CreatePostVC {
	...
	func timerTick() {
		...
		let positivelrZero = max(total_RecordedTime_In_Secs, 0)
		let progress = Float(positivelrZero) / Float(trimmedTime) / 10
